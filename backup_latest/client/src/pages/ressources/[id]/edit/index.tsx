...
interface EditRessourceProps {
  id: string;
}

const EditRessource: React.FC<EditRessourceProps> = ({ id }) => {
  const [, navigate] = useLocation();
  const { toast } = useToast();
  const queryClient = useQueryClient();

  const { data: ressource, isLoading: isLoadingRessource, error: ressourceError } = useQuery({
  ...
  // Met à jour le formulaire quand les données sont chargées
  useEffect(() => {
    if (ressource) {
      form.reset({
        nom: ressource.nom,
        ...
        caracteristiques: ressource.caracteristiques || {},
      });

      // Remplir les champs pour les caractéristiques
      if (ressource.caracteristiques) {
        if (ressource.caracteristiques.marque) {
          form.setValue('marque', ressource.caracteristiques.marque);
        }
        if (ressource.caracteristiques.modele) {
          form.setValue('modele', ressource.caracteristiques.modele);
        }
        if (ressource.caracteristiques.annee) {
          form.setValue('annee', ressource.caracteristiques.annee.toString());
        }
        if (ressource.type === 'vehicule' && ressource.caracteristiques.immatriculation) {
          form.setValue('immatriculation', ressource.caracteristiques.immatriculation);
        }
      }
    }
  }, [ressource, form]);
  ...
  const mutation = useMutation({
    mutationFn: (values: FormValues) => {
      return apiRequest(`/api/ressources/${id}`, {
        method: 'PUT',
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: [`/api/ressources/${id}`] });
      toast({
  ...
  const onSubmit = (values: FormValues) => {
    ...
    if (values.type === 'vehicule') {
      caracteristiques.marque = form.watch('marque') || '';
      caracteristiques.modele = form.watch('modele') || '';
      caracteristiques.immatriculation = form.watch('immatriculation') || '';
      caracteristiques.annee = form.watch('annee') ? parseInt(form.watch('annee')) : null;
    ...
    return (
      <div className="container mx-auto py-6">
        <div className="bg-red-50 text-red-800 p-4 rounded-md my-8">
          ...
          <p className="text-sm">{(ressourceError as Error)?.message || 'Ressource non trouvée'}</p>
          <Button 
            variant="outline" 
            className="mt-4" 
  ...
  return (
    <div className="container mx-auto py-6">
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Modifier une ressource</h1>
      ...
      <div className="grid grid-cols-1 gap-6">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Mettre à jour les détails de cette ressource
            </CardDescription>
          </CardHeader>
          <CardContent>
            <Form {...form}>
