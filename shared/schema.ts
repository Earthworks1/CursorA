...
export const taches = pgTable("taches", {
  ...
  piloteId: integer("pilote_id"), // Responsable du pilotage de la tâche
  intervenantId: integer("intervenant_id"), // Personne chargée de la réalisation
  created_at: timestamp("created_at").defaultNow(),
...
export const tachesRelations = relations(taches, ({ one, many }) => ({
  ...
  pilote: one(users, {
    fields: [taches.piloteId],
    references: [users.id],
  }),
  intervenant: one(users, {
