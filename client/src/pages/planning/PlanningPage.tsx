...
function TaskCard({ tache, dragListeners, isDragging, onDelete }) {
  return (
    <div
      {...dragListeners}
...
export default function PlanningPage() {
  ...
  function handleDragEnd(event) {
    ...
    setDraggedTask(null);
    if (!over) return;
    const [intervenantId, date] = over.id.split("|");
    setLoading(true);
    fetch("/api/planning-tasks", {
      method: "PATCH",
  ...
  return (
    <div className="p-6">
      ...
      <DndContext collisionDetection={closestCenter} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
        ...
        <DragOverlay>
          {draggedTask ? <TaskCard tache={draggedTask} isDragging={true} onDelete={() => handleDeleteTask(draggedTask.id)} /> : null}
        </DragOverlay>
      </DndContext>
    </div>
  );
...
function DraggableTask({ tache, isDragging, onDelete }) {
